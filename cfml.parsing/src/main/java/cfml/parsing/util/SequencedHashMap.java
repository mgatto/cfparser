 ...
 * {@link java.util.List#remove(Object)} and {@link java.util.Map#remove(Object)}).
 * 
 * <P>
 * This class is not thread safe. When a thread safe implementation is required, use
 * {@link Collections#synchronizedMap(Map)} as it is documented, or use explicit synchronization controls.
 * 
 * @since 2.0
...
public class SequencedHashMap implements CaseSensitiveMap, Cloneable, Externalizable {
	...
	private class OrderedIterator implements Iterator {
		 ...
		 *                if there isn't a "last element" to be removed. That is, if {@link #next()} has never been
		 *                called, or if {@link #remove()} was already called on the element.
		 * 
		 * @exception ConcurrentModificationException
		 *                if a modification occurs in the underlying map.
		 **/
